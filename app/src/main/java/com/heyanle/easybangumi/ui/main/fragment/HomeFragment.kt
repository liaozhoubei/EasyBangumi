...
class HomeFragment : Fragment(R.layout.fragment_home) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.currentSourceIndex.observe(viewLifecycleOwner){
            ...
            homeIndex = it
            binding.homeSource.text = getString(R.string.current_home_source, SourceParserFactory.homeLabel()[it])
            if(isChangeSource){
                refresh()
                isChangeSource = false
            }
        }
        binding.refreshLayout.setColorSchemeColors(getAttrColor(requireContext(), com.google.android.material.R.attr.colorSecondary))
